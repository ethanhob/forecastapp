...
function App() {
  ...
  // these were initialised as strings but only given values once the requests are done
  const [forecast, setForecast] = useState(null);

  const search = evt => {
    // formatting helps. the issue was the fetches embedded and now they are async
    if (evt.key === 'Enter') {
  ...
  return (
    <div className="App">
      <main>
        ...
        {(weather && forecast) ? (
          // typeof weather.location != "undefined" && typeof forecast.forecast != undefined then run below else return nothing
          <div>
